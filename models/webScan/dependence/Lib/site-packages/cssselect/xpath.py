...
class GenericTranslator(object):
    ...
    #: The attribute used for ``:language()`` depends on the document language:
    #: http://www.w3.org/TR/selectors/#lang-pseudo
    lang_attribute = 'xml:language'
    ...
    def xpath_lang_function(self, xpath, function):
        if function.argument_types() not in (['STRING'], ['IDENT']):
            raise ExpressionError(
                "Expected a single string or ident for :language(), got %r"
                % function.arguments)
        value = function.arguments[0].value
        return xpath.add_condition(
            "language(%s)" % (self.xpath_literal(value)))
...
class HTMLTranslator(GenericTranslator):
    ...
    def xpath_lang_function(self, xpath, function):
        ...
        value = function.arguments[0].value
        return xpath.add_condition(
            "ancestor-or-self::*[@language][1][starts-with(concat("
