...
# Scan - Web Application Security Scanner
...
Scan is basically an update for WAScan making it compatible for python >= 3.4
...
- [Scan - Web Application Security Scanner](#sitadel---web-application-security-scanner)
...
```bash
git clone https://github.com/shenril/Scan.git
cd webScan
pip3 install .
python Sanner.py --help
```
...
```bash
Sanner.py [-h] [-r {0,1,2}] [-ua USER_AGENT] [--redirect]
...
| ---------------------- | ----------------------------------------------------------------------------------------- |
| -h, --help         | Display help |
| -r, --risk {0,1,2}        | Decide the risk level you want Scan to run (some attacks won't be executed)          |
| -ua, --user-agent       | User agent used for the HTTP request of the attacks          |
| --redirect      | Indicates to Scan to follow the 302 request for page redirection                                          |
| --no-redirect             | Indicates to Scan **NOT** to follow the 302 request for page redirection                |
...
| -c, --config           | Specify the config file for Scan scan, default one is in config/config.yml      |
| -v, --verbosity          | Increase the default verbosity of the logs, for instance: -v , -vv, -vvv                                                      |
| --version          | Show Scan version                                                                       |
